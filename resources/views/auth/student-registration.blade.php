<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        @csrf
        <x-text-input type="hidden" name="role" value="student" />
        ...
        <!-- Standards -->
        <x-input-label for="standards" :value="__('Select Class')" class="mt-4" />
        <div id="standards" class="flex flex-wrap">
            @foreach ($standards as $standard)
                <div class="flex w-32 items-center gap-2 p-2">
                    <x-text-input id="standard{{ $standard->id }}" class="p-2" type="radio" name="standard_id"
                        value="{{ $standard->id }}" />
