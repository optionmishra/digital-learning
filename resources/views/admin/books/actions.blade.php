<div class="d-flex">
    <button class="px-2 py-2 btn btn-link nav-link d-flex align-items-center edit-btn" type="button" title="Edit"
        ...
        data-row-data="{{ json_encode(['id' => $book->id, 'series_id' => $book->series_id, 'name' => $book->name, 'about' => $book->about, 'board_id' => $book->board->id, 'standard_id' => $book->standard->id, 'subject_id' => $book->subject->id, 'author_id' => $book->author->id]) }}">
