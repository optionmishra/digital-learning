<div class="d-flex">
    <button class="px-2 py-2 btn btn-link nav-link d-flex align-items-center edit-btn" type="button" title="Edit"
        data-coreui-toggle="modal" data-coreui-target="#notificationStore"
        data-update-route="{{ route('admin.notifications.store') }}"
        data-row-data="{{ json_encode(['id' => $notification->id, 'title' => $notification->title, 'description' => $notification->description, 'start_date' => $notification->start_date, 'end_date' => $notification->end_date, 'is_active' => $notification->is_active]) }}">
    ...
    <button class="px-2 py-2 btn btn-link nav-link d-flex align-items-center" type="button" title="Delete"
        ...
        data-delete-route="{{ route('admin.notifications.destroy', $notification->id) }}">
