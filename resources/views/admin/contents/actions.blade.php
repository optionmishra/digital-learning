<div class="d-flex">
    <button class="btn btn-link nav-link d-flex align-items-center edit-btn px-2 py-2" type="button" title="Edit"
        ...
        data-row-data="{{ json_encode(['id' => $content->id, 'title' => $content->title, 'tags' => $content->tags, 'img_type' => $content->img_type, 'img_url' => $content->img, 'src_type' => $content->src_type, 'url' => $content->src, 'price' => $content->price, 'about' => $content->about, 'standard_id' => $content->standard_id, 'subject_id' => $content->subject_id, 'book_id' => $content->book_id, 'topic_id' => $content->topic_id]) }}">
