<div class="d-flex">
    <button class="px-2 py-2 btn btn-link nav-link d-flex align-items-center edit-btn" type="button" title="Edit"
        data-coreui-toggle="modal" data-coreui-target="#assessmentStore"
        data-update-route="{{ route('admin.assessments.store') }}"
        data-row-data="{{ json_encode(['id' => $assessment->id, 'name' => $assessment->name, 'standard_id' => $assessment->standard_id, 'subject_id' => $assessment->subject_id, 'book_id' => $assessment->book_id, 'type' => $assessment->type, 'duration' => $assessment->duration]) }}">
    ...
    <button class="px-2 py-2 btn btn-link nav-link d-flex align-items-center" type="button" title="Delete"
        ...
        data-delete-route="{{ route('admin.assessments.destroy', $assessment->id) }}">
